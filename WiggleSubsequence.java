  ...
  class Solution {
    ...
    public int wiggleMaxLength9(int[] nums) {
      ...
      for (int i = 1; i < nums.length; i++) {
        ...
        if (neg == nums[i - 1] > nums[i]) {
          firstNeg++;
          neg = !neg;
        }
      }
      return Math.max(firstPos, firstNeg);
    }
    
    // 自解精简（贪心） 摆动序列最大长度 = max（第一个差为正的摆动序列最大长度，第一个差为负的摆动序列最大长度）
    public int wiggleMaxLength8(int[] nums) {
      ...
      int neg = -1;
      for (int i = 1; i < nums.length; i++) {
        if (pos * (nums[i] - nums[i - 1]) > 0) {
          firstPos++;
          pos = -pos;
        }
        if (neg * (nums[i] - nums[i - 1]) > 0) {
          firstNeg++;
          neg = -neg;
        }
      }
     ...
     *
     * 状态转移：
     *  nums[i-1] < nums[i]：up[i]   = down[i-1] + 1, 假设 down[i] 表示的最长摆动序列的最远末尾元素下标小于 i，设为 j，那么 nums[j:i] 一定是递增的
     *                       down[i] = down[i-1]
     *  nums[i-1] > nums[i]：down[i] = up[i-1] + 1
     *                       up[i]   = up[i-1]
     *  nums[i-1] = nums[i]：up[i]   = up[i-1]
     *                       down[i] = down[i-1]
     */
    ...
    /*
     * ☆☆☆☆☆ 贪心
     * 摆动序列长度即为数组中「峰」和「谷」的总数（包括端点、平峰和平谷都只算一个）
     */
    public int wiggleMaxLength(int[] nums) {
      int ans = 1;
      for (int i = 1, preDiff = 0, curDiff = 0; i < nums.length; i++) {
